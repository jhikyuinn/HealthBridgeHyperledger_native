...
function Information({user,navigation}) {
  ...
  const Edit=()=>{
    navigation.navigate('Owner Edit Profile',{user:user})
  }
  const Charge=()=>{
    navigation.navigate('Owner Charge Balance',{user:user})
  }
  ...
  const SignOut=()=>{
    ...
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text style={{fontSize:25,fontWeight:"bold"}}>User Information {"\n"} </Text>
